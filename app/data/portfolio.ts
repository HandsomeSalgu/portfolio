...
export const projects: Project[] = [
  ...
  },
  {
    slug: 'betting-royal',
    title: 'BettingRoyal',
    summary: '방 코드로 입장해 라운드제로 겨루는 Unity 멀티플레이 파티 게임',
    period: '2026.08.24 - 진행 중',
    role: '인프라 · 백엔드 단독 담당 (CI/CD · 배포 · REST API · 데이터 파이프라인)',
    teamSize: 6,
    techStack: [
      'Java',
      'Spring Boot 3',
    ...
    highlights: [
      'AWS EC2에 nginx · Spring Boot · MySQL · Jenkins 4컨테이너 Compose 스택 구축 — MySQL 포트를 열지 않고 nginx 프록시 하나로 외부 진입 경로 단일화',
      'Jenkins 경로별 자동 배포 구성, 프록시를 거친 /health 폴링으로 실제 서비스 가능 상태를 배포 성공 기준으로 설정',
      '상시 가동이 아닌 Unity 러너가 pending 으로 팀 전체 머지를 잠그던 문제를, 러너 온라인 여부 프로브 + 동적 자식 파이프라인으로 해결',
      '멱등키(matchId, accountId) 기반 매치 결과 · 메달 원장 API 설계 — 응답 유실 후 재시도에도 잔액이 한 번만 오르도록 보장 (7 엔드포인트 · 테스트 42건)',
      'JPA save() 의 merge 동작으로 발생한 메달 이중 지급 버그를 저사양 CI 컨테이너에서 재현해 규명, 네이티브 INSERT + 생성자 제거로 재발 경로 차단',
      'Excel → 검증 → MessagePack 데이터 빌드 파이프라인과 Addressables 원격 배포 자동화로 기획자의 xlsx 커밋만으로 수치 반영',
    ],
  },
]
